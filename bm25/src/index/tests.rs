...
mod test_module {
    ...
    fn test_index_manager_create() {
        ...
        let _manager = IndexManager::create(&path).expect("Failed to create index manager");
        assert!(path.exists());
    }
    ...
    fn test_search_basic() {
        ...
        let (_results, max_score) = result.expect("Search failed");
    ...
    fn test_schema_to_document() {
        ...
        assert!(doc.field_values().count() >= 3);
    }
}
