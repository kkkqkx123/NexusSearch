...
compile_error!("At least one storage backend must be enabled: 'storage-tantivy' or 'storage-redis'");
...
impl StorageManagerBuilder {
    /// 创建默认存储管理器（只读）
    #[cfg(feature = "storage-tantivy")]
    pub fn build_tantivy(config: crate::storage::tantivy::TantivyStorageConfig) -> Result<StorageManager> {
    ...
    /// 创建可变存储管理器
    #[cfg(feature = "storage-tantivy")]
    pub fn build_mutable_tantivy(config: crate::storage::tantivy::TantivyStorageConfig) -> Result<MutableStorageManager> {
        let storage = TantivyStorage::new(config);
