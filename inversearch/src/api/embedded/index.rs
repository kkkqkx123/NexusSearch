...
impl EmbeddedIndex {
    ...
    pub fn search_with_limit(
        ...
        let embedded_results: Vec<EmbeddedSearchResult> = result.results
            ...
            .enumerate()
            .map(|(idx, doc_id)| {
                let content = self.document_store.get(&doc_id)
                    .cloned()
                    .unwrap_or_default();
    ...
    pub fn stats(&self) -> EmbeddedIndexStats {
        EmbeddedIndexStats {
            ...
            index_path: self.config.index_path.as_ref().map(|p| p.to_string_lossy().to_string()),
        }
    }
    ...
    pub fn save(&self) -> Result<()> {
        let path = self.config.index_path.as_ref()
            .ok_or_else(|| crate::error::InversearchError::Config(
                "No index path configured".to_string()
            ))?;
        self.save_to(path)
    }
        ...
        let docs_data = if self.config.store_documents {
            serde_json::to_vec(&self.document_store).map_err(|e| {
                crate::error::InversearchError::Serialization(format!(
                    "Failed to serialize document store: {}", e
                ))
            })?
        } else {
            Vec::new()
    ...
    pub fn load_from(&mut self, path: impl AsRef<std::path::Path>) -> Result<()> {
        ...
        let metadata_len = u64::from_le_bytes(
            file_content[offset..offset + 8].try_into().map_err(|_| {
                crate::error::InversearchError::Deserialization("Invalid file format".to_string())
            })?
        ) as usize;
        offset += 8;

        let metadata: IndexFileMetadata = serde_json::from_slice(
            &file_content[offset..offset + metadata_len]
        ).map_err(|e| {
        ...
        self.index = Index::from_binary(&file_content[offset..offset + index_len], &serialize_config)?;
        offset += index_len;

        if metadata.has_document_store {
            let docs_len = u64::from_le_bytes(
                file_content[offset..offset + 8].try_into().map_err(|_| {
                    crate::error::InversearchError::Deserialization("Invalid file format".to_string())
                })?
            ) as usize;
            ...
            self.document_store = serde_json::from_slice(
                &file_content[offset..offset + docs_len]
            ).map_err(|e| {
                crate::error::InversearchError::Deserialization(format!(
                    "Failed to deserialize document store: {}", e
                ))
            })?;
        }
...
mod tests {
    ...
    fn test_embedded_index_save_and_load() {
        ...
        loaded_index.load_from(&index_path).expect("Failed to load index");
