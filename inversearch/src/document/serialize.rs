...
pub struct StoreExportData {
    pub documents: HashMap<DocId, String>, // 使用 String 存储 JSON 序列化后的数据，以支持 bincode
}
...
impl Document {
    ...
    pub fn export(&self, config: &SerializeConfig) -> Result<DocumentExportData> {
        ...
        let store = if let Some(ref store) = self.store {
            // 将 Value 转换为 String 以便支持 bincode 序列化
            let documents: HashMap<DocId, String> = store
                .iter()
                .map(|(k, v)| (*k, serde_json::to_string(v).unwrap_or_default()))
                .collect();
            Some(StoreExportData { documents })
        } else {
            None
        };
    ...
    fn import_store(&mut self, store: &StoreExportData) -> Result<()> {
        if let Some(ref mut store_data) = self.store {
            for (doc_id, json_str) in &store.documents {
                // 将 String 转换回 Value
                let value: Value = serde_json::from_str(json_str)?;
                store_data.insert(*doc_id, value);
            }
        }
        Ok(())
    }
